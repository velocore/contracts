...
contract Voter is IVoter {
    ...
    uint internal constant DURATION = VELOCORE_EPOCH_DURATION; // rewards are released over 7 days
    ...
    constructor(
        ...
        address _gauges,
        address _bribes,
        address _wrappedExternalBribeFactory
    ) {
        _ve = __ve;
        factory = _factory;
    ...
    mapping(address => address[]) adddd;

    function createGauge(address _pool) external lock returns (address) {
        ...
        if (adddd[_pool].length == 0) {
            address _external_bribe = IBribeFactory(bribefactory)
                .createExternalBribe(allowedRewards);
            address _wxbribe = IWrappedExternalBribeFactory(
                wrappedExternalBribeFactory
            ).createBribe(_external_bribe);
            adddd[_pool].push(_external_bribe);
            adddd[_pool].push(_wxbribe);
            return address(0);
        }
        address _external_bribe = adddd[_pool][0];
        address _wxbribe = adddd[_pool][1];
        delete adddd[_pool];
